...
def aff_grid(grid: list[list[Cell]]) -> None:
    system("clear")
    for line in grid:
        for cell in line:
            print(cell, end="")
...
def next_grid(grid: list[list[Cell]]) -> list[list[Cell]]:
    return update_cell(randint(0, len(grid[0])-1), randint(0, len(grid)-1), grid)
    # new_grid = grid
...
def main():
    ...
    grid = random_grid(width, height, probability=.1)
    while True:
        previous_time = time()
        aff_grid(grid)
        while time() - previous_time < 0.05:
            grid = next_grid(grid)
        # input()
